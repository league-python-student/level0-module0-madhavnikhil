...
if __name__ == '__main__':
    ...
    if Questiontwo == "5.4" :
        messagebox.showinfo(title=window, message="You are correct")
        Score = Score+1
    Questionthree = simpledialog.askstring(title=window, prompt="How much is 8.6 divided by 2.8")
    if Questionthree == "3.07" :
        messagebox.showinfo(title=window, message="You are correct")
        Score = Score+1
    # After all the questions have been asked, tell the user their final score
    messagebox.showinfo(title=window, message=str(Score))
